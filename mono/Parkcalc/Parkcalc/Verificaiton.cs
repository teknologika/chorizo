...
namespace Parkcalc
{
    public static class Verificaiton
    {
        public static void VerifyParkingPrice(decimal expected)
        {
            Regex regex = new Regex(@"\s{2,}",RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline);
            string actual = regex.Replace(Physical.GetResult().Trim()," ");
            string[] splitResult = actual.Split(' ');

            Assert.AreEqual(expected, decimal.Parse(splitResult[1]));

            //AssertEqual(days,Convert.ToInt32(splitResult[2].Replace("(","")),"Verification of number of days.");
